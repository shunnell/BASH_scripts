...
APP_SLACK_WEBHOOK=https://hooks.slack.com/services/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
APP_SLACK_CHANNEL=general
APP_SLACK_USERNAME=Raspberry
APP_SLACK_ICON_EMOJI=:rasp:
...
init_params() {
  ...
  if [ -z "${APP_SLACK_WEBHOOK:-}" ]; then
    echo 'error: Please configure Slack environment variable: '
    echo '  APP_SLACK_WEBHOOK' 
    exit 2
  fi
